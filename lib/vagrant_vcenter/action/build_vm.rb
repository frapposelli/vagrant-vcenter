      ...
      class BuildVM
        ...
        def call(env)
          ...
          if env[:machine].box.name.include? '/'
            ...
            box_name = env[:machine].box.name.gsub(%r{///}, '-')
          else
            box_file = env[:machine].box.name
            box_name = box_file
          end

          box_to_search = if cfg.template_folder_name.nil?
                            box_name
                          else
                            "#{cfg.template_folder_name}/#{box_name}"
                          end

          cfg.compute_rp = if cfg.resourcepool_name
                             cfg.compute.resourcePool.resourcePool.find {
                              |f| f.name == cfg.resourcepool_name
                             }
                           else
                             cfg.compute.resourcePool
                           end
          ...
          if cfg.enable_vm_customization
            ...
            if cfg.prep_type.downcase == 'linux'
              ...
              raise Errors::GuestCustomNotSupported,
                    :type => cfg.prep_type
            end
          ...
          vm_to_search = if cfg.folder_name.nil?
                           vm_target.to_s
                         else
                           "#{cfg.folder_name}/#{vm_target}"
                         end
          ...
          env[:machine].id = cfg.datacenter.find_vm(
            vm_to_search
          ).config.uuid or raise Errors::VMNotFound,
